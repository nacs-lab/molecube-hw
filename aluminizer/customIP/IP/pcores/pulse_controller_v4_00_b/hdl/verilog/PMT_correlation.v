...
// TR 05/18/2010: Allow parameterization of N_BITS and N_BINS.  
//		  It would be nice if this design didn't hog the whole FPGA.
//		  Only one adder is really needed instead of many parellel ones.
//
// 1/25/2011 TR: Control reset_counters from SW registers.  
//               Data from several detections can be combined.
// 
////////////////////////////////////////////////////////////////////////////////

//removefromqueue_f <= configdata_in[0];
//clockdivider_f <= configdata_in[1:4];
//reset_counters <= configdata_in[5];

module PMT_correlation(clk, reset, configdata_in, configwrite_in, gate_in, data_out, dataready_out, pmt_in, sync_in);

   parameter N_BINS = 16; // number of bins 
   parameter N_BITS = 8; // number of bits per counter must be 4, 8, 16, or 32

   input  wire 		  clk;
   input  wire 		  reset;
   
   input  wire [0:5]	  configdata_in;
   input  wire		  configwrite_in;
	
   input  wire		  gate_in;
   
   output wire [0:(N_BINS*N_BITS - 1)] data_out;
...
always @(posedge clk)
	if (reset) begin
	    ...
	    clockcounter <= clockcounter + 4'b0001;
	    dividedclk <= 1'b0;
	end
	else begin
	    clockcounter <= 4'h0;
...
assign clk2 = clk; // (clockdivider_f > 1) ? dividedclk : clk;
...
wire counting2; //0: not counting.  1: counting
...
parameter N_BITS = 8; // number of bits per counter
...
input  wire 					sync_i;
  ...
  if (reset_i) count_o <= {(N_BITS){1'b0}};
	else if (photon_f) count_o <= count_o + 1'b1;
